...
export default function PackageIntegration() {
  ...
  // 패키지 연동 북마크릿 JavaScript 코드 (SlickGrid 기반 - 실제 CRM 구조에 맞춤)
  const bookmarkletCode = `javascript:(function(){console.log('🚀 북마크릿 시작 (SlickGrid 버전)');var hash=window.location.hash;var searchData=null;if(hash.indexOf('package_data=')!==-1){try{var encoded=hash.split('package_data=')[1];var decoded=decodeURIComponent(atob(encoded));searchData=JSON.parse(decoded);console.log('✅ URL 데이터 파싱 성공:',searchData);}catch(e){console.error('❌ 데이터 파싱 오류:',e);alert('URL 데이터 파싱 실패: '+e.message);return;}}else{console.log('⚠️ URL에 package_data 없음, localStorage에서 확인');var stored=localStorage.getItem('crm_package_search');if(stored){searchData=JSON.parse(stored);console.log('✅ localStorage에서 데이터 복원:',searchData);}else{alert('검색 데이터가 없습니다. 환자 관리 시스템에서 최신화 버튼을 먼저 클릭하세요.');return;}}if(searchData&&searchData.customerNumber){var searchInput=document.querySelector('[id*="srch_clnt_no"]');if(searchInput){console.log('🔍 검색 입력란 발견, 고객번호 입력:',searchData.customerNumber);searchInput.value=searchData.customerNumber;searchInput.focus();var enterEvent=new KeyboardEvent('keydown',{key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true,cancelable:true});searchInput.dispatchEvent(enterEvent);console.log('⌨️ Enter 이벤트 발송');}else{console.error('❌ 검색 입력란을 찾을 수 없음');alert('검색 입력란을 찾을 수 없습니다. CRM 패키지 관리 페이지에 있는지 확인하세요.');}setTimeout(function(){try{var resultRow=document.querySelector('[title="'+searchData.customerNumber+'"]');if(resultRow){console.log('✅ 검색 결과 발견, 더블클릭 시도');var dblClickEvent=new MouseEvent('dblclick',{bubbles:true,cancelable:true,view:window});resultRow.dispatchEvent(dblClickEvent);console.log('🖱️ 더블클릭 이벤트 발송');setTimeout(function(){try{console.log('📊 SlickGrid 데이터 추출 시작');var formatDate=function(dateStr){if(!dateStr)return'';var cleaned=dateStr.replace(/[^0-9]/g,'');if(cleaned.length===8){return cleaned.substring(0,4)+'-'+cleaned.substring(4,6)+'-'+cleaned.substring(6,8);}else if(cleaned.length===6){return'20'+cleaned.substring(0,2)+'-'+cleaned.substring(2,4)+'-'+cleaned.substring(4,6);}return dateStr;};var getSlickGridData=function(gridIdPattern){var transactions=[];console.log('\\n🔍 SlickGrid 검색:',gridIdPattern);var grid=document.querySelector('[id*="'+gridIdPattern+'"]');console.log('→',grid?'✅ 발견 ('+grid.id+')':'❌ 없음');if(grid){var canvas=grid.querySelector('.grid-canvas');if(!canvas){console.log('❌ grid-canvas를 찾을 수 없음');return transactions;}var rows=canvas.querySelectorAll('.slick-row');console.log('전체 행 개수:',rows.length);var validRows=Array.from(rows).filter(function(row){var firstCell=row.querySelector('.slick-cell');if(!firstCell)return false;var label=firstCell.querySelector('lable');if(!label)return false;var title=label.getAttribute('title')||label.textContent.trim();return title!=='합계'&&title!=='소계';});console.log('필터링 후 행 개수:',validRows.length);validRows.forEach(function(row,idx){var cells=row.querySelectorAll('.slick-cell');if(cells.length>=2){var dateLabel=cells[0].querySelector('lable');var valueLabel=cells[1].querySelector('lable');var noteLabel=cells.length>=3?cells[2].querySelector('lable'):null;if(dateLabel&&valueLabel){var dateText=(dateLabel.getAttribute('title')||dateLabel.textContent.trim()).replace(/-/g,'');var valueText=(valueLabel.getAttribute('title')||valueLabel.textContent.trim()).replace(/,/g,'');var noteText=noteLabel?(noteLabel.getAttribute('title')||noteLabel.textContent.trim()):'';console.log('행',idx,': date=',dateText,', value=',valueText,', note=',noteText);var value=parseFloat(valueText);if(!isNaN(value)&&value!==0&&dateText){var formattedDate=formatDate(dateText);console.log('  ✅ 저장: date=',formattedDate,', value=',value);transactions.push({date:formattedDate,value:value,note:noteText});}}else{console.log('행',idx,': lable 태그 없음');}}});console.log('→',gridIdPattern,'최종 데이터:',transactions.length,'건');}return transactions;};var depositIncome=getSlickGridData('gridPkgActCdA01');var depositUsage=getSlickGridData('gridPkgActCdA02');var rewardIncome=getSlickGridData('gridPkgActCdB01');var rewardUsage=getSlickGridData('gridPkgActCdB02');var countInput=getSlickGridData('gridPkgActCdC01');var countUsage=getSlickGridData('gridPkgActCdC02');var packageData={customerNumber:searchData.customerNumber,depositIncome:depositIncome,depositUsage:depositUsage,rewardIncome:rewardIncome,rewardUsage:rewardUsage,countInput:countInput,countUsage:countUsage,lastSyncedAt:new Date().toISOString()};console.log('\\n📦 최종 패키지 데이터:');console.log('  예치금 입금:',depositIncome.length,'건');console.log('  예치금 사용:',depositUsage.length,'건');console.log('  적립금 입금:',rewardIncome.length,'건');console.log('  적립금 사용:',rewardUsage.length,'건');console.log('  횟수 입력:',countInput.length,'건');console.log('  횟수 사용:',countUsage.length,'건');console.log('전체 데이터:',packageData);var totalCount=[depositIncome.length,depositUsage.length,rewardIncome.length,rewardUsage.length,countInput.length,countUsage.length].reduce(function(a,b){return a+b;},0);if(totalCount===0){alert('⚠️ 데이터를 찾지 못했습니다.\\n\\n페이지가 완전히 로드된 후 다시 시도하거나, 콘솔(F12)에서 상세 로그를 확인하세요.');return;}localStorage.setItem('crm_package_result',JSON.stringify(packageData));if(window.opener&&!window.opener.closed){console.log('✅ 부모 창으로 데이터 전송');try{window.opener.postMessage({type:'crm-package-data',data:packageData},'*');alert('데이터 추출 완료! ('+totalCount+'건)\\n\\n창이 자동으로 닫힙니다.');window.close();}catch(e){console.error('postMessage 오류:',e);alert('데이터 전송 실패. localStorage에 저장되었습니다.');}}else{console.log('⚠️ 부모 창 없음, localStorage에 저장');alert('데이터 추출 완료! ('+totalCount+'건)\\nlocalStorage에 저장되었습니다.\\n\\n환자 관리 시스템으로 돌아가서 확인하세요.');if(searchData.appUrl){window.location.href=searchData.appUrl;}}}catch(e){console.error('❌ 패키지 데이터 추출 오류:',e);console.error('스택:',e.stack);alert('데이터 추출 중 오류가 발생했습니다:\\n'+e.message+'\\n\\n콘솔(F12)에서 상세 정보를 확인하세요.');}},3000);}else{console.error('❌ 검색 결과를 찾을 수 없음: title="'+searchData.customerNumber+'"');alert('검색 결과를 찾을 수 없습니다.\\n고객번호: '+searchData.customerNumber+'\\n\\n수동으로 고객을 검색해보세요.');}}catch(e){console.error('❌ 더블클릭 오류:',e);alert('검색 결과 처리 중 오류: '+e.message);}},2000);}else{alert('searchData가 없거나 customerNumber가 비어있습니다.');}})();`;
