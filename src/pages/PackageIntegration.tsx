...
export default function PackageIntegration() {
  ...
  const bookmarkletCode = `javascript:(function(){var hash=window.location.hash;var searchData=null;if(hash.indexOf('package_data=')!==-1){try{var encoded=hash.split('package_data=')[1];var decoded=decodeURIComponent(atob(encoded));searchData=JSON.parse(decoded);}catch(e){console.error('데이터 파싱 오류:',e);return;}}else{console.log('검색 데이터가 URL에 없습니다.');return;}if(searchData&&searchData.customerNumber){var searchInput=document.querySelector('[id*="srch_clnt_no"]');var searchBtn=document.querySelector('[id*="btn_srch"]');if(searchInput){searchInput.value=searchData.customerNumber;}if(searchBtn){searchBtn.click();setTimeout(function(){try{var getGridSum=function(gridId){var total=0;var grid=document.querySelector('[id*="'+gridId+'"]');if(grid){var rows=grid.querySelectorAll('tbody tr');rows.forEach(function(row){var cells=row.querySelectorAll('td');if(cells.length>=2){var value=cells[1].textContent.trim().replace(/,/g,'');var num=parseFloat(value);if(!isNaN(num)){total+=num;}}});}return total;};var depositIncome=getGridSum('grd_list_dpst_pay_01');var depositUsage=getGridSum('grd_list_dpst_pay_02');var rewardIncome=getGridSum('grd_list_mmbr_pnt_01');var rewardUsage=getGridSum('grd_list_mmbr_pnt_02');var countInput=getGridSum('grd_list_thpy_03');var countUsage=getGridSum('grd_list_thpy_04');var packageData={customerNumber:searchData.customerNumber,depositTotal:depositIncome,depositUsed:depositUsage,depositBalance:depositIncome-depositUsage,rewardTotal:rewardIncome,rewardUsed:rewardUsage,rewardBalance:rewardIncome-rewardUsage,countTotal:countInput,countUsed:countUsage,countBalance:countInput-countUsage,lastSyncedAt:new Date().toISOString()};if(window.opener&&!window.opener.closed){window.opener.postMessage({type:'crm-package-data',data:packageData},'*');window.close();}else{var appUrl=searchData?searchData.appUrl:window.location.origin+'/patient-list';localStorage.setItem('crm_package_data',JSON.stringify(packageData));window.open(appUrl+'?package=import','_blank');}}catch(e){console.error('패키지 데이터 추출 오류:',e);}},1000);}}})();`;
