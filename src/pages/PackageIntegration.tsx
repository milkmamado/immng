...
export default function PackageIntegration() {
  ...
  const bookmarkletCode = `javascript:(function(){var hash=window.location.hash;var searchData=null;if(hash.indexOf('package_data=')!==-1){try{var encoded=hash.split('package_data=')[1];var decoded=decodeURIComponent(atob(encoded));searchData=JSON.parse(decoded);console.log('✅ URL 데이터 파싱 성공:',searchData);}catch(e){console.error('데이터 파싱 오류:',e);return;}}else{console.log('검색 데이터가 URL에 없습니다.');return;}if(searchData&&searchData.customerNumber){var searchInput=document.querySelector('[id*="srch_clnt_no"]');if(searchInput){console.log('🔍 검색 입력란 발견, 고객번호 입력:',searchData.customerNumber);searchInput.value=searchData.customerNumber;var enterEvent=new KeyboardEvent('keydown',{key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true});searchInput.dispatchEvent(enterEvent);}setTimeout(function(){try{var resultRow=document.querySelector('[title="'+searchData.customerNumber+'"]');if(resultRow){console.log('✅ 검색 결과 발견, 더블클릭 시도');var dblClickEvent=new MouseEvent('dblclick',{bubbles:true,cancelable:true,view:window});resultRow.dispatchEvent(dblClickEvent);setTimeout(function(){try{console.log('📊 그리드 데이터 추출 시작');var formatDate=function(dateStr){if(!dateStr)return'';var cleaned=dateStr.replace(/[^0-9]/g,'');if(cleaned.length===8){return cleaned.substring(0,4)+'-'+cleaned.substring(4,6)+'-'+cleaned.substring(6,8);}return cleaned;};var getGridData=function(gridId){var transactions=[];var grid=document.querySelector('[id*="'+gridId+'"]');console.log('그리드 검색:',gridId,'→',grid?'발견':'없음');if(grid){var tbody=grid.querySelector('tbody');if(!tbody){console.log(gridId,'tbody 없음');return transactions;}var rows=Array.from(tbody.querySelectorAll('tr')).filter(function(row){return!row.classList.contains('slick-group')&&!row.classList.contains('slick-group-totals');});console.log(gridId,'행 개수:',rows.length);rows.forEach(function(row,idx){var cells=row.querySelectorAll('td');if(cells.length>=2){var dateText='';var valueText='';var noteText='';if(gridId.indexOf('dpst_pay_02')!==-1||gridId.indexOf('mmbr_pnt_02')!==-1||gridId.indexOf('thpy_04')!==-1){dateText=cells[1]?cells[1].textContent.trim():'';valueText=cells[2]?cells[2].textContent.trim().replace(/,/g,''):'0';noteText=cells[3]?cells[3].textContent.trim():'';}else{dateText=cells[0]?cells[0].textContent.trim():'';valueText=cells[1]?cells[1].textContent.trim().replace(/,/g,''):'0';noteText=cells[2]?cells[2].textContent.trim():'';}var value=parseFloat(valueText);console.log(gridId,'행',idx,':',dateText,'→',valueText,'=',value,noteText);if(!isNaN(value)&&value!==0&&dateText&&dateText!=='합계'){var formattedDate=formatDate(dateText);transactions.push({date:formattedDate,value:value,note:noteText});}}});console.log(gridId,'최종 데이터:',transactions);}return transactions;};var depositIncome=getGridData('grd_list_dpst_pay_01');var depositUsage=getGridData('grd_list_dpst_pay_02');var rewardIncome=getGridData('grd_list_mmbr_pnt_01');var rewardUsage=getGridData('grd_list_mmbr_pnt_02');var countInput=getGridData('grd_list_thpy_03');var countUsage=getGridData('grd_list_thpy_04');var packageData={customerNumber:searchData.customerNumber,depositIncome:depositIncome,depositUsage:depositUsage,rewardIncome:rewardIncome,rewardUsage:rewardUsage,countInput:countInput,countUsage:countUsage,lastSyncedAt:new Date().toISOString()};console.log('📦 최종 패키지 데이터:',packageData);if(window.opener&&!window.opener.closed){console.log('✅ 부모 창으로 데이터 전송');window.opener.postMessage({type:'crm-package-data',data:packageData},'*');window.close();}else{console.error('❌ 부모 창을 찾을 수 없음');alert('원본 창을 찾을 수 없습니다.');}}catch(e){console.error('패키지 데이터 추출 오류:',e);alert('데이터 추출 중 오류: '+e.message);}},2000);}else{console.error('❌ 검색 결과를 찾을 수 없음');alert('검색 결과를 찾을 수 없습니다.');}}catch(e){console.error('더블클릭 오류:',e);}},1500);}})();`;
