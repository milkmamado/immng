...
export default function CRMBookmarklet() {
  ...
  const bookmarkletCode = `javascript:(function(){var hash=window.location.hash;var searchData=null;if(hash.indexOf('crm_data=')!==-1){try{var encoded=hash.split('crm_data=')[1];var decoded=decodeURIComponent(atob(encoded));searchData=JSON.parse(decoded);}catch(e){console.error('데이터 파싱 오류:',e);return;}}else{console.log('검색 데이터가 URL에 없습니다.');return;}if(searchData){var searchBtn=document.querySelector('[id*="btn_bsClntNmPopUp"]');if(searchBtn){searchBtn.click();setTimeout(function(){var popup=document.querySelector('.win_popup-wapper');if(popup){var nameInput=popup.querySelector('[id*="srch_clnt_nm"]');var phoneInput=popup.querySelector('[id*="srch_hp_telno"]');var branchSelect=popup.querySelector('[id*="srch_bnch_cd"]');var popupSearchBtn=popup.querySelector('[id*="btn_srch"]');if(branchSelect)branchSelect.value='';if(nameInput)nameInput.value=searchData.name||'';if(phoneInput)phoneInput.value=searchData.phone||'';if(popupSearchBtn){popupSearchBtn.click();}}},500);}}document.addEventListener('dblclick',function(e){setTimeout(function(){console.log('=== CRM 데이터 추출 디버깅 ===');var findInputWithValue=function(pattern){var inputs=document.querySelectorAll('input[id*="'+pattern+'"]');for(var i=0;i<inputs.length;i++){var val=inputs[i].value.trim();if(val){return inputs[i];}}return null;};var findSelectWithValue=function(pattern){var selects=document.querySelectorAll('select[id*="'+pattern+'"]');for(var i=0;i<selects.length;i++){var select=selects[i];var option=select.options[select.selectedIndex];if(option&&option.value){return select;}}return selects.length>0?selects[0]:null;};var findTextareaWithValue=function(pattern){var textareas=document.querySelectorAll('textarea[id*="'+pattern+'"]');for(var i=0;i<textareas.length;i++){var val=textareas[i].value.trim();if(val){return textareas[i];}}return textareas.length>0?textareas[0]:null;};var getValue=function(pattern){var el=findInputWithValue(pattern);return el?el.value.trim():'';};var getSelectedText=function(pattern){var select=findSelectWithValue(pattern);if(!select)return'';var option=select.options[select.selectedIndex];return option?option.text.trim():'';};var getTextareaValue=function(pattern){var el=findTextareaWithValue(pattern);return el?el.value.trim():'';};var getAllHospitalSelects=function(){var allSelects=document.querySelectorAll('select[id*="_org_hspt_cd"]');console.log('전체 병원 관련 select 수:',allSelects.length);allSelects.forEach(function(sel,idx){var isUpCategory=sel.id.indexOf('_up_')!==-1;var selectedOption=sel.options[sel.selectedIndex];var selectedText=selectedOption?selectedOption.text.trim():'(선택안됨)';var selectedValue=selectedOption?selectedOption.value:'';console.log('Select #'+idx+':',sel.id,'| 대분류:',isUpCategory,'| 선택된값:',selectedText,'| value:',selectedValue);});};getAllHospitalSelects();var getHospitalCategory=function(){var select=document.querySelector('select[id*="_up_org_hspt_cd"]');console.log('이전병원 대분류 select:',select?select.id:'찾을수없음');if(!select)return'';var option=select.options[select.selectedIndex];var result=option?option.text.trim():'';console.log('이전병원 대분류 결과:',result);return result;};var getHospitalBranch=function(){var select=document.querySelector('select[id*="_org_hspt_cd"]:not([id*="_up_"])');console.log('이전병원 중분류 select:',select?select.id:'찾을수없음');if(!select)return'';var option=select.options[select.selectedIndex];var result=option?option.text.trim():'';console.log('이전병원 중분류 결과:',result);return result;};var name=getValue('bs_clnt_nm');if(name){var data={name:name,customer_number:getValue('bs_clnt_no'),resident_number_masked:getValue('bs_rrn'),phone:getValue('bs_hp_telno'),gender:getValue('bs_sex'),age:getValue('bs_spec_age')||getValue('bs_age'),address:(getValue('bs_up_addr1')+' '+getValue('bs_ref_addr1')).trim(),visit_motivation:getSelectedText('cmhs_motv_cd'),diagnosis_category:getSelectedText('dgns_cd'),diagnosis_detail:getSelectedText('dgns_detl_cd'),hospital_category:getHospitalCategory(),hospital_branch:getHospitalBranch(),crm_memo:getTextareaValue('cms_call_memo'),special_note_1:getTextareaValue('rmk_1'),special_note_2:getTextareaValue('rmk_2'),treatment_memo_1:getTextareaValue('mdcr_memo_cnts_1'),treatment_memo_2:getTextareaValue('mdcr_memo_cnts_2')};console.log('최종 추출 데이터:',data);if(window.opener&&!window.opener.closed){window.opener.postMessage({type:'crm-patient-data',data:data},'*');window.close();}else{var appUrl=searchData?searchData.appUrl:window.location.origin+'/first-visit';localStorage.setItem('crm_patient_data',JSON.stringify(data));window.open(appUrl+'?crm=import','_blank');}}else{console.error('환자명을 찾을 수 없습니다');}},500);});})();`;
