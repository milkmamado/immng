...
export default function CRMBookmarklet() {
  ...
  const bookmarkletCode = `javascript:(function(){alert('북마크릿이 시작되었습니다!');var appUrl='https://c1b1e147-d88f-49c7-a031-a4345f1f4a69.lovableproject.com/first-visit';var searchData=localStorage.getItem('crm_search_data');if(searchData){alert('검색 데이터를 찾았습니다: '+searchData);var data=JSON.parse(searchData);var searchBtn=document.querySelector('[id*="btn_bsClntNmPopUp"]');if(searchBtn){alert('돋보기 버튼을 찾았습니다!');searchBtn.click();setTimeout(function(){var popup=document.querySelector('.win_popup-wapper');if(popup){alert('팝업창이 열렸습니다!');var nameInput=popup.querySelector('[id*="srch_clnt_nm"]');var phoneInput=popup.querySelector('[id*="srch_hp_telno"]');var branchSelect=popup.querySelector('[id*="srch_bnch_cd"]');var popupSearchBtn=popup.querySelector('[id*="btn_srch"]');if(branchSelect)branchSelect.value='';if(nameInput)nameInput.value=data.name||'';if(phoneInput)phoneInput.value=data.phone||'';if(popupSearchBtn){popupSearchBtn.click();alert('검색되었습니다. 환자를 더블클릭하세요.');}localStorage.removeItem('crm_search_data');}else{alert('팝업창을 찾을 수 없습니다.');}},500);}else{alert('돋보기 버튼을 찾을 수 없습니다.');}}else{alert('검색 데이터가 없습니다. 먼저 \"조회\" 버튼을 클릭하세요.');}document.addEventListener('dblclick',function(e){setTimeout(function(){var findInput=function(pattern){var inputs=document.querySelectorAll('input[id*="'+pattern+'"]');return inputs.length>0?inputs[0]:null;};var findSelect=function(pattern){var selects=document.querySelectorAll('select[id*="'+pattern+'"]');return selects.length>0?selects[0]:null;};var findTextarea=function(pattern){var textareas=document.querySelectorAll('textarea[id*="'+pattern+'"]');return textareas.length>0?textareas[0]:null;};var getValue=function(pattern){var el=findInput(pattern);return el?el.value.trim():'';};var getSelectedText=function(pattern){var select=findSelect(pattern);if(!select)return'';var option=select.options[select.selectedIndex];return option?option.text.trim():'';};var getTextareaValue=function(pattern){var el=findTextarea(pattern);return el?el.value.trim():'';};var name=getValue('bs_clnt_nm');if(name){var data={name:name,customer_number:getValue('bs_clnt_no'),resident_number_masked:getValue('bs_rrn'),phone:getValue('bs_hp_telno'),gender:getValue('bs_sex'),age:getValue('bs_spec_age')||getValue('bs_age'),address:(getValue('bs_up_addr1')+' '+getValue('bs_ref_addr1')).trim(),visit_motivation:getSelectedText('cmhs_motv_cd'),diagnosis_category:getSelectedText('dgns_cd'),diagnosis_detail:getSelectedText('dgns_detl_cd'),hospital_category:getSelectedText('org_hspt_cd'),crm_memo:getTextareaValue('cms_call_memo')};if(window.opener&&!window.opener.closed){window.opener.postMessage({type:'crm-patient-data',data:data},'*');alert('환자 정보가 전송되었습니다.\\n원래 창에서 확인하세요.');window.close();}else{localStorage.setItem('crm_patient_data',JSON.stringify(data));window.open(appUrl+'?crm=import','_blank');}}},500);});})();`;
