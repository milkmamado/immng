...
export default function CRMBookmarklet() {
  ...
  // 북마크릿 JavaScript 코드 - CRM 자동화 (디버깅용: 창 닫기 비활성화, 대기시간 1500ms)
  const bookmarkletCode = `javascript:(function(){var hash=window.location.hash;var searchData=null;if(hash.indexOf('crm_data=')!==-1){try{var encoded=hash.split('crm_data=')[1];var decoded=decodeURIComponent(atob(encoded));searchData=JSON.parse(decoded);}catch(e){console.error('데이터 파싱 오류:',e);return;}}else{console.log('검색 데이터가 URL에 없습니다.');return;}if(searchData){var searchBtn=document.querySelector('[id*="btn_bsClntNmPopUp"]');if(searchBtn){searchBtn.click();setTimeout(function(){var popup=document.querySelector('.win_popup-wapper');if(popup){var nameInput=popup.querySelector('[id*="srch_clnt_nm"]');var phoneInput=popup.querySelector('[id*="srch_hp_telno"]');var branchSelect=popup.querySelector('[id*="srch_bnch_cd"]');var popupSearchBtn=popup.querySelector('[id*="btn_srch"]');if(branchSelect)branchSelect.value='';if(nameInput)nameInput.value=searchData.name||'';if(phoneInput)phoneInput.value=searchData.phone||'';if(popupSearchBtn){popupSearchBtn.click();}}},500);}}document.addEventListener('dblclick',function(e){console.log('[디버그] 더블클릭 감지');setTimeout(function(){console.log('[디버그] 1500ms 후 데이터 추출 시작');var findInput=function(pattern){var inputs=document.querySelectorAll('input[id*="'+pattern+'"]');console.log('[디버그] findInput('+pattern+'):', inputs.length+'개 발견');return inputs.length>0?inputs[0]:null;};var findSelect=function(pattern){var selects=document.querySelectorAll('select[id*="'+pattern+'"]');console.log('[디버그] findSelect('+pattern+'):', selects.length+'개 발견');return selects.length>0?selects[0]:null;};var findTextarea=function(pattern){var textareas=document.querySelectorAll('textarea[id*="'+pattern+'"]');console.log('[디버그] findTextarea('+pattern+'):', textareas.length+'개 발견');return textareas.length>0?textareas[0]:null;};var getValue=function(pattern){var el=findInput(pattern);var val=el?el.value.trim():'';console.log('[디버그] getValue('+pattern+'):', val);return val;};var getSelectedText=function(pattern){var select=findSelect(pattern);if(!select)return'';var option=select.options[select.selectedIndex];return option?option.text.trim():'';};var getTextareaValue=function(pattern){var el=findTextarea(pattern);return el?el.value.trim():'';};var name=getValue('bs_clnt_nm');console.log('[디버그] 고객명:', name);if(name){var data={name:name,customer_number:getValue('bs_clnt_no'),resident_number_masked:getValue('bs_rrn'),phone:getValue('bs_hp_telno'),gender:getValue('bs_sex'),age:getValue('bs_spec_age')||getValue('bs_age'),address:(getValue('bs_up_addr1')+' '+getValue('bs_ref_addr1')).trim(),visit_motivation:getSelectedText('cmhs_motv_cd'),diagnosis_category:getSelectedText('dgns_cd'),diagnosis_detail:getSelectedText('dgns_detl_cd'),hospital_category:getSelectedText('org_hspt_cd'),crm_memo:getTextareaValue('cms_call_memo')};console.log('[디버그] 추출된 데이터:', data);if(window.opener&&!window.opener.closed){console.log('[디버그] postMessage 전송');window.opener.postMessage({type:'crm-patient-data',data:data},'*');console.log('[디버그] 창 닫기 생략 (디버깅용)');}else{console.log('[디버그] localStorage 저장');var appUrl=searchData?searchData.appUrl:window.location.origin+'/first-visit';localStorage.setItem('crm_patient_data',JSON.stringify(data));window.open(appUrl+'?crm=import','_blank');}}else{console.log('[디버그] 고객명을 찾을 수 없음');}},1500);});})();`;
