-- Drop all existing policies for package_transactions
...
-- Create new INSERT policies: Masters can insert all, Managers only for their patients
CREATE POLICY "Masters can insert all package transactions"
...
CREATE POLICY "Managers can insert package transactions for their patients"
ON public.package_transactions
FOR INSERT
WITH CHECK (
  EXISTS (
...
-- UPDATE: Keep restricted to managers for their own patients only
CREATE POLICY "Managers can update package transactions for their patients"
ON public.package_transactions
FOR UPDATE
USING (
  EXISTS (
...
-- DELETE: Keep restricted to managers for their own patients only  
